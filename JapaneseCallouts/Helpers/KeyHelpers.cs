...
internal static class KeyHelpers
{
    ...
    => NativeFunction.Natives.UPDATE_ONSCREEN_KEYBOARD<int>() is not 0 && (Game.IsKeyDown(Modifier) || Modifier is Keys.None) && Game.IsKeyDown(Main);

    internal static bool IsKeysDownRightNow(Keys Main, Keys Modifier = Keys.None)
    => NativeFunction.Natives.UPDATE_ONSCREEN_KEYBOARD<int>() is not 0 && (Game.IsKeyDownRightNow(Modifier) || Modifier is Keys.None) && Game.IsKeyDownRightNow(Main);
